...
| F33 / CFL-4 | KAN-189 | ✅ **Effective dating on position changes — CFL-4 CLOSED with no history rewritten.** The convention is now stated: **every effective-dated period is half-open `[effective_from, effective_to)`**, so `effective_to` is the first day NOT covered (ADR-020). Read that way, the "one-day overlap" the finding described **was never in the data** — closing with `effective_to = CURRENT_DATE` and opening with `effective_from = CURRENT_DATE` is exactly adjacent. **The defect was the absence of a stated convention, not the rows**, which is why the resolution is a documented rule plus a small code change and *not* a data migration. `org_change_requests.effective_date` now carries one date that drives both ends of the boundary, so they cannot drift; `fmt_period()` / `fmt_last_day()` are the only sanctioned renderers (a grep-assert fails the build if a template prints `effective_to` raw). **DEF-42-2 closed in the same change** — the manager re-point set `is_current = FALSE` and nothing else, leaving superseded reporting lines with a NULL end date; migration 10 repairs the 2 rows the old code left, deriving each end from its successor's `effective_from`. Verified against real Postgres: **0 overlapping periods** in `employee_org_assignments` and **0** among `SOLID_LINE` relationships under `daterange(from, to, '[)') &&`. **Unblocks `AC-185-07`, so KAN-185 can close.** Per-company window values are **not** yet delivered — that table is W1 — and `_dating_window()` is the single named seam |
| F32 | KAN-203 | ✅ **Segregation of duties in the org-change chain** (`DEF-42-4` + `DEF-42-5`, P0, raised against **EP27**) — subject ≠ initiator and subject ≠ decider, universal, `SYSTEM_ADMIN` included; both refusals audited as `ORG_CHANGE_SELF_ACTION_REFUSED` (`SECURITY`, `FAILED`). Every surface brought into line: no **Transfer…** on your own profile or directory row, your own org-tree card not draggable (still a drop target), `list_pending` omits requests about you, and `_step_approver_user_ids` excludes the subject so your own move never reaches your bell or badge. `CLAUDE.md` invariant 2 rewritten in the same commit. Named reproductions in `tests/test_transfer_entry_point.py` §8, verified failing with the guards removed |
...
| F32 | P0 | Security / Domain logic | **Segregation of duties absent in the org-change chain — one person could raise and approve their own move** (`DEF-42-4` + `DEF-42-5`, raised against **EP27**). `_can_initiate_for` returned `True` for any `HR_ADMIN`/`PORTAL_ADMIN`/`SYSTEM_ADMIN` **without ever comparing the initiator to the subject**, and `decide()` compared the decider to the step and the company and to nothing else — never to `req['employee_id']`. On the seeded Acme chain, where one user holds both `PORTAL_ADMIN` and `HR_ADMIN`, that carried a self-raised request from `PENDING` to applied unaided. **`CLAUDE.md`'s headline invariant said this was impossible; the detailed rule beneath it granted the exemption** — a class of defect this review did not look for, because it read the two as one statement. Note the contrast with the *rest* of the domain logic, which the summary above rates strong: the flaw is not in the chain's sequencing but in who is allowed to stand in it. | `app/routes/org_change.py:55-64` (pre-fix); `app/services/org_change_service.py:222-249` (pre-fix); the only self-check in the product was a **display** helper at `app/routes/employees.py:126-131`, which said so in its own docstring | Two mandatory guards, refused outright and never flag-and-override: **subject ≠ initiator** (ahead of the admin exemption) and **subject ≠ decider** (any level, any role, `SYSTEM_ADMIN` included); audit each refusal as `ORG_CHANGE_SELF_ACTION_REFUSED` / `retention_class='SECURITY'`; make every surface agree, so no control is offered that the guard then refuses. **Generalise the review lesson:** where a rule has a headline and a mechanism, audit them as one claim. | S |
