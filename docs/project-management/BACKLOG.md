...
| EP42 | Compensation, Job Architecture & Pay Equity | **W0** KAN-203 · KAN-204 · KAN-208 · KAN-188 · KAN-189 · **W1** KAN-190 · KAN-191 · KAN-207 · **W2** KAN-199 · KAN-206 · KAN-210 · KAN-193 · KAN-194 · KAN-195 · KAN-209 · **W3** KAN-196 · KAN-197 · KAN-198 · KAN-192 · **W4** KAN-200 · KAN-201 · KAN-205 · KAN-202 | ⬜ Planned · **BG7** (new) — W0 builds in **S2**, W1–W4 in **S3** (D-004). **23 stories in 5 waves** (A1 · Wave-5 reconciliation · **A2**). Carries **KAN-203, a P0 Critical pre-existing defect fix**, and **unblocks KAN-185** |
| EP43 | Annual Compensation Review Cycle | KAN-216 · KAN-217 · KAN-211 · KAN-212 · KAN-213 · KAN-214 · KAN-215 | ⬜ Planned · **BG7** · **new in A2, extended by A3** — the annual hike cycle, now including the **performance input** the owner authorised. **Sibling of EP42, sequenced after EP42 W4**, stage **S3**. Outline depth — entering **S1**, not build. **The natural descope if S3 runs long** |
| EP44 | Performance Management | — | ⬜ **Named, not entered.** **BG7** · **A3.** Full performance management — cycles, goals, self-assessment, calibration, ratings history. **Exists only if the owner picks option (b)** in the sizing put to him (`EP42_SPM_SCOPE_AND_DECISIONS.md` §21.2): ≈20–25 stories, 12–16 weeks, and it would precede EP43, pushing the hike cycle out by roughly a quarter. **SPM recommends (a) — the 2-story performance *input* in EP43 — and the team is building that meanwhile** |
...
>     KAN-204 ─┤                                    EP33 debt, gates the first EP42 screen
>     KAN-208 ─┤ parallel                           synthetic gender, so Check B has data
>     KAN-188 ─┤                                    tenant switch + non-session resolver
>     KAN-189 ─┘ (second track, unblocks KAN-185)   effective dating
> S3  KAN-190 → KAN-191 → KAN-207                   ladder + expectations, levels assigned, the roadmap
>     KAN-199 → KAN-206 → KAN-210 → KAN-193 → KAN-194 → KAN-195 → KAN-209
>                                                   markets, pay points, ladder re-basing, the record,
>                                                   visibility, backfill, then fit the steps and open the gate
...
### W0 — platform enablers, two debts and one data task (stage **S2**)
...
### W2 — pay points, the compensation record, and the fitted ladder (stage **S3**) — R1, R2
...
| ⬜ KAN-209 | As **HR**, I want every employee's **step** fitted to what they are actually paid, reviewed, and only then released — so the equity check starts clean instead of flagging the whole company. (**A1 §14.2.5** · **created by CFL-42-41** · **the W2 exit gate**) | **Fits the step to the pay, not the pay to the step:** each employee is placed at **the step whose pay point is nearest their actual pay**, on **relative (ratio) distance**; **ties fit downward** (fitting up manufactures a *primary* `PAY_BELOW_STEP` from a tie, fitting down at most a secondary one) and **the fit is deterministic** — a fit that flips between runs makes every downstream finding unreproducible, and that assertion matters more than which way ties break. This sounds circular and is exactly right for an initial load: the ladder is being fitted to reality, because reality came first. **Why this is a separate story and why it is last in W2:** fitting needs KAN-206's pay points **and** real pay data from KAN-193/195, so the original plan — fitting inside KAN-191 in W1 — would have run it a wave before either existed, dropping everyone at `.0` and producing precisely the alert storm the fitting exists to prevent (**CFL-42-41**, the SPM's own build-order defect). **Delivers the review screen** (146 people, HR confirming or overriding each fit — the same adoption-cliff problem as the title mapping) and **releases the ladder-fitted-and-reviewed gate**, which Check A′ waits on. The gate **projects the finding count before it is released** — "releasing this will raise N findings" — so HR meets the number on a screen rather than in an inbox; under the nearest-step rule that number should be **zero** from a clean fit, which makes the projection a **regression detector** rather than an apology. Employees with no pay record stay at `.0` and are listed for manual placement. **Also reports ladder-description completeness** ("28 of 34 steps described") — A2 §4 says expectations are defined *"for all positions and for all defined levels"*, making completeness a launch expectation rather than a nice-to-have, and this gate is the only instrument anyone will have for R-18. **Also the W2 exit gate for KAN-208's distribution assertion** (CFL-42-42) — at least two comparison groups satisfy Check B's minimums. | **Must · P1** |
...
| ⬜ KAN-205 | As a **customer admin**, I want a salary band for each level in each pay market, so I can see whether a pay figure is sane for that level at all. (D3.5 · **repositioned by A1**) | **Repositioned, not resized.** A band is no longer the comparison basis — the step pay point (KAN-206) is. A band is the level's **min/max envelope**: *"is this pay sane for this level at all?"*, which is a genuinely different question from *"does it match the step?"*. `(job_level, pay_market) → min, midpoint, max, currency, effective_from`. **Compa-ratio** computed and displayed to holders of `compensation:r`. **Out-of-band pay is allowed, flagged and requires a reason — never hard-blocked**: red-circled legacy pay and genuine market premiums exist, and hard-blocking pushes the decision off-system, which is the problem we started with. Also delivers **band context on the approval surface** — an approver deciding a pay change should see where it lands in the range. A **soft** dependency of KAN-200: the engine is complete and correct without bands. Every band change audited. | **Should · P2** |
...
**Build order:** `KAN-216 → KAN-217 → KAN-211 → KAN-212 → KAN-213 → KAN-214 → KAN-215` — **the performance-input pair goes first**, honouring the owner's *"implement this first if required if this the blocker"*.
...
| Story ID | User Story | Acceptance Criteria | Priority |
|----------|-----------|---------------------|----------|
| ⬜ KAN-216 | As a **customer admin**, I want to define what our performance bands are and what each is worth, so the hike policy is ours and defensible. (**A3-2 option (a)** · **A3-1**) | Company-defined **ordinal bands** (e.g. below / meets / exceeds / outstanding), each mapping to a **configured multiplier** on the cycle guideline. Authored on an **admin page by the employer**, per company. **Nothing predetermined and nothing suggested** — no shipped band set, no shipped multipliers, no "typical is 3–5%" helper text, no placeholder. A company that has not authored a policy **cannot open a cycle**. **Versioned, auditable, and inspectable without exposing any individual's data** — because performance-related pay is works-council territory in Germany and the Nordics (where the seed data lives), and a works council may need to review and agree the bands *before* the cycle operates. That promotes **OQ-8 from a launch consideration to a design input**, and this requirement is where it lands. The *policy* of what "exceeds" is worth sits at company level; the *judgement* sits with the manager (KAN-217). | Must · P2 |
| ⬜ KAN-217 | As a **manager**, I want to record how each of my people performed this cycle, so the hike reflects it — and I want that used for nothing else. (**A3-2 option (a)**) | A manager enters **one band per employee per cycle**. **Said plainly rather than dressed up: this is a performance rating, and it is the one line EP43 deliberately crosses** — §14.5 is superseded by owner decision. **Single-purpose, enforced the way pay is:** written in a cycle, read by that cycle, used for nothing else; **absent from the profile, the directory, the org tree, reporting and every export**; **no rating-history surface** — one band per employee per cycle, because "performance over time" is the drift; row-scoped like pay so a manager records for their direct reports and sees nobody else's; its own feature code and its own **negative-visibility suite** asserting absence from the payload. **Not shown to the employee by default** — communicating a rating is a conversation, not a screen. **What stays out:** goals, objectives, the review cycle itself, calibration, forced curves, scoring rubrics, 360 or peer input, competency assessment. **The ladder is still not this** — ADR-025's forbidden-column table stands unchanged, and no rating, score, achievement or progress field may appear on `job_step_expectations` or `employee_step_roadmaps`. | Must · P2 |
...
| ⬜ KAN-212 | As a **customer admin**, I want to set what funds the cycle and what differentiates it, so the percentages come from somewhere I can defend. | The three inputs the owner named: **market movement — per pay market**, because Stockholm's market is not Tallinn's and a single company-wide number would manufacture findings the way a company-level `monthly_payments_per_year` would · **company affordability** · the **per-employee performance band** — **defined in KAN-216 and entered in KAN-217**, not duplicated here. **Nothing about the policy is predetermined** (A3-1): the guideline, the bands and the multipliers are all employer-authored, and the product ships and suggests no values. | Must · P2 |
| ⬜ KAN-213 | As a **manager**, I want to distribute the cycle across my team, so the people who took on most get most. | Per-employee proposals generated from `current base × (1 + guideline × band multiplier)`; a manager worksheet showing a **running total against the budget**, over-budget **warned and reasoned, never blocked** (invariant 3). Row-scoped exactly as pay is — a manager proposes for their direct reports and sees nobody else's numbers. | Must · P2 |
