...
> KAN-205, KAN-212 and KAN-217 are withdrawn and recorded rather than reused. **KAN-241 is taken (A7, the readiness outcome). The next free ID is KAN-242.**
...
| | **✅ KAN-190 DONE (10 Aug 2026).** Families → levels → steps, with the ladder's dangerous number defended three ways. **`step_count` counts increments ABOVE entry**, so 5 yields six steps `.0`–`.5` (the owner's own `2.0 … 2.5`): a **database trigger** enforces the cross-table bound — **TD-21 is closed, not accepted**, because a `step_no` of 6 on a 5-step level is a pay point compounded six times, not a cosmetic error; `step_total` is computed once in the service so no template or JS repeats the arithmetic; and the form **previews what the number means** ("That gives 6 steps: 2.0 to 2.5") before it is saved. Verified in real Postgres: step 5 accepted, step 6 refused with the rule in the message. **No default on `step_count`** — column, API and form all require an answer, and the demo ladder deliberately has levels of 3, 5 and 2 steps to show why. **Ordinal immutable once occupied** (ADR-017b), counting **historic** assignments because a past record still points at the coordinate; renaming stays free; reducing steps below an occupied one is refused naming who is affected; and the empty state warns **before** the ladder is built. **Two gates on one screen, and the third deliberately absent** — read `job_architecture:r` (seeded to **every** role, so an employee can read their own step and the next), configure `org_structure:w`, and **no write route uses `job_architecture:w`** because that is roadmap authoring, not ladder editing (CFL-42-35, both halves, grep-asserted). The feature's **label and description** say what write actually grants, since a grant reading "Job Architecture: write" would mislead at the moment it is made. **Tenancy enforced by composite FKs**, not by service code — a cross-tenant family insert is refused by the database (verified), as is whitespace masquerading as authored content. Expectations are **sparse and never inherited**; `level_steps()` returns every step so an incomplete ladder cannot look complete; completeness is `4/9` with the gaps named, never a bare percentage. **`TestNoAssessmentColumnsOnTheLadder` fails the build on any `score`/`rating` column** in either the migration or `schema.sql` — the §14.5 / R-17 boundary made enforceable. **`STEP_NOT_ASSESSED` is nullable and distinct from step 0** (A6), ready for KAN-191. First EP42 screen, and it **calls KAN-204's shell primitives rather than reinventing them** — `announce()`, `escH()`, the global focus ring, dialog semantics with focus return and Esc; asserted, because if the first of ten screens reinvents them all ten will. **4,774 pytest · browser 110/110 · vacation 39/39**, plus a CI-style rebuild. ⚠ **R-18 stands and cannot be automated away:** nothing here can test whether the ladder text is any good, so *"the ladder reads as a real description of the work"* is on the Demo Gate's human list — and the demo seed leaves one level **undescribed on purpose** so the half-authored state is demoable. | |
...
> **Decision D-009 — 2026-08-10 — the three shape questions for performance management.**
> *Context:* EP44's requirements exercise opened under D-008 (Option C). Three choices changed the
> **shape** of the build rather than its wording, so they were put to the owner with a recommended
> default each and an explicit note that none of them blocked.
>
> | # | Question | Default put to him | His answer |
> |---|---|---|---|
> | 1 | Cycle cadence, and is it tied to the pay round? | Annual, mid-term check-in, independent of the pay round | **"Once a year"** — default confirmed |
> | 2 | Is probation a real cycle or just a label? | Just a label; no probation cycles, no probation screens | **"No probation a real cycle"** — read as *no probation **as** a real cycle*, i.e. default confirmed |
> | 3 | Does an employee see their manager's written assessment? | Yes, after the manager submits | **"Yes"** — default confirmed |
>
> **⚠ On (2), the reading is recorded because the phrasing was ambiguous.** *"No probation a real
> cycle"* was taken as **"no probation as a real cycle"** — probation stays a **label**, consistent
> with the other two answers both confirming their defaults, and consistent with EP42's KAN-192 already
> offering `PROBATION_REVIEW` as a `review_context` rather than a process. **If that reading is wrong,
> probation becomes its own cycle type with its own dates and outcomes, and KAN-219/220 change
> materially** — so it is flagged here rather than buried, and it was flagged to him in the same breath
> as being acted on.
>
> *What each answer settles:*
> - **(1) Annual.** A cycle's period is a company-defined **year**, so `period_type` is not a
>   configurable enum in this cycle and the model does not carry quarterly/semi-annual variants. **At
>   most one cycle that is not `CLOSED`** per company — annual makes that a real constraint rather
>   than an arbitrary one. Independent of the pay round means a cycle carries **no pay period
>   reference**, which keeps EP43's dependency one-directional.
> - **(2) Probation is a label.** No probation entity, no probation cycle, no probation screens. A
>   probationer is simply **in the annual cycle** like everybody else, and "this step change happened
>   at a probation review" stays a free choice of `review_context` on EP42's KAN-192.
> - **(3) The employee sees it.** After the manager submits, and **never before their own
>   self-assessment is in** (KAN-224's order is the point of it). Consistent with the ladder
>   transparency he asked for twice. **Recorded as culturally load-bearing:** it is awkward to reverse
>   once people have written assessments expecting privacy, so a later change is a policy decision with
>   a migration, not a settings toggle.
>
...
> specification** — checked before writing this, not assumed.
>
> His words:
>
> > *"During performance evaluation there will section which will Business Goals and Development goals.
> > The Business goals and development during start of will be discussed and agreed with employee.
> > Employee will continue to add milestones acheived through out the year. Now at the end during
> > performance evaluation Manager will comment wheteher he each Business and Development has been
> > succesfully acheived abd based on that satisfaction did the employee provide reating : Below
> > expection, Meeting expectation, Above expectation and Beyond expecatiation. Before employee provides
> > his rating employee needs to submit his side of the story and rating. Now during perofomance
> > evalutaion discussion the point of discssuin on the each goals and finally Should employee focus on
> > current role, Employee can be ready to next level in next 12-24 months, Ready to move for next level."*

### ⚠️ A correction owed first — engineering overstated the "no score" boundary

Engineering told him *"it cannot ever hold a score or rating"*. **That was about the LADDER tables only
(EP42's roadmaps and step expectations), and it was said in a way that sounded like the product would
never have ratings.** It will. Ratings have always belonged in EP44 (KAN-226 specifies a company-defined
ordinal scale). The boundary that actually holds is narrower and worth stating exactly:

| Object | Rating? | Why |
|---|---|---|
| Step expectations, step roadmaps (**EP42**) | **Never** | They describe a job and what somebody should do next. A score attached to a person's development plan is a profiling artefact. |
| Performance rating (**EP44**) | **Yes — this is where it lives** | A human's judgement of a period's work, on a named scale, with the human recorded. |
| Anything **computed** from other fields | **Never, anywhere** | A weighted total or a derived rating is an automated decision about a person (GDPR Art. 22 / EU AI Act). |

**The line is not "no ratings". It is: a human states a rating; a system never calculates one.**

### What A7 changes — the four-stage sequence, stated

1. **Start of year** — Business Goals and Development Goals are agreed with the employee.
2. **Through the year** — the employee adds **milestones achieved** against those goals.
3. **At evaluation, in this order and enforced:**
   a. the **employee** submits their side of the story **and their own rating**;
   b. **only then** may the manager comment per goal and give **their** rating.
4. **The discussion** covers each goal, and **concludes with a readiness statement**.

### The stories A7 amends

| Story | What changes |
|---|---|
| **KAN-221** (goals) | Goals gain a **type: `BUSINESS` or `DEVELOPMENT`** — they are two named sections, not a tag. Agreed with the employee at the start; the agreement is recorded. |
| **KAN-222** (check-ins) | Reframed as **milestones achieved**: dated, authored by the **employee**, against a specific goal, visible to the manager. **Not approved one by one** — a year-long approval queue is how this dies; the manager weighs them at review time. Still no percentage complete and no RAG status. |
| **KAN-224** (self-assessment) | The employee submits **a narrative AND a self-rating** on the same scale. **This now GATES the manager's rating, not just its visibility** — stronger than D-009(3) recorded, and corrected here. |
| **KAN-225** (manager assessment) | **Per goal**, the manager records an outcome and a comment; **then** an overall rating. The overall rating is **the manager's judgement informed by the per-goal outcomes — never computed from them.** |
| **KAN-226** (scale) | His four labels — **Below expectation · Meeting expectation · Above expectation · Beyond expectation** — ship as **the default scale**, still company-configurable and versioned as already specified. |
| **KAN-228** (the conversation) | The discussion is recorded per goal, and the **readiness statement is its conclusion** — recorded *after* the conversation, which is what he described. |

### KAN-241 — NEW STORY: the readiness outcome

**Nothing in the specification covered this.** It is the most consequential thing in A7, because it is
the point where a review touches the job ladder.

| Story ID | User Story | Acceptance Criteria | Priority |
|----------|-----------|---------------------|----------|
| ⬜ KAN-241 | As a **manager and employee**, I want the review to conclude with where this person stands on the ladder, so the outcome is stated rather than left implied. (**A7**, new) | **A fixed three-value outcome, deliberately NOT company-configurable:** `FOCUS_ON_CURRENT_ROLE` · `READY_IN_12_24_MONTHS` · `READY_FOR_NEXT_LEVEL`. Fixed because unlike the rating scale — which is about a *period's work* and is properly a company's own vocabulary — these three are **structurally tied to the product's own promotion mechanism** (stay / progressing / level change), and a tenant renaming them would break the link to KAN-197. **Recorded as the conclusion of the discussion**, not before it: it is an outcome of a conversation, which is what the owner described. **`READY_FOR_NEXT_LEVEL` NEVER auto-creates anything.** It is a *stated view* that a human then acts on by raising a level change through the existing governed path — chain approval (KAN-197), four eyes on money (KAN-198), Art. 22 attestation (KAN-232). An automatic promotion from a review outcome is precisely the automated decision the whole epic is built to avoid. **`READY_IN_12_24_MONTHS` is a forward-looking statement about a person**, which is closer to profiling than a judgement of past work: it is therefore **written by a human, never computed, and never usable as a filter** — it may not rank people, gate access to an opportunity, or feed any list that a human does not author. **Mandatory to answer, nothing pre-selected** (D4c), because an unanswered outcome means the question fell on the floor. **Visible to the employee**, consistent with D-009(3) — and because it is recorded after the discussion, they hear it from their manager before they read it. Changeable afterwards only as an **audited amendment with a reason**. **The audit row carries the outcome and no amount.** | **Must · P1** |
...
> solved once, but the answer must be different: the ordering exists to *protect the employee*, so
> letting the manager simply proceed would defeat its purpose.
> **Recommendation:** the cycle carries a **self-assessment deadline**. After it, HR may record
> `SELF_ASSESSMENT_NOT_SUBMITTED` — the employee is notified, the manager may then proceed, and **the
...
> not.
> **Recommendation:** three values — `ACHIEVED` · `PARTIALLY_ACHIEVED` · `NOT_ACHIEVED` — because
...
**The manager sees the employee's self-rating before giving their own.** That is what he asked for and
it is the point — the manager should consider it. It also **anchors** them, which is a known effect of
this ordering. Recorded as a property of the chosen design, not as an objection: some systems hide the
self-rating for exactly this reason, and he has decided otherwise.
...
| | **📋 ACCEPTANCE CRITERIA — KAN-219, written 10 Aug 2026 (D-009).** **AC-219-01** A cycle belongs to one company and covers one **year**, whose start month is company-configurable (default January) — D-009(1) settles annual, so there is **no `period_type` enum** and the model carries no quarterly or semi-annual variant. Adding one later is a migration, and pretending to support it now would be a configurable field with one legal value. **AC-219-02** Lifecycle `DRAFT → OPEN → IN_REVIEW → CALIBRATION → CLOSED`, forward-only. **AC-219-03 At most ONE cycle per company that is not `CLOSED`** — annual makes this a real constraint, not an arbitrary cap; enforced by a **partial unique index**, not by service code, for the same reason `uq_eja_one_current` is (two open cycles make "which cycle am I in?" ambiguous and every downstream reader picks one arbitrarily). **AC-219-04** The window is **half-open `[opens_on, closes_on)`** on ADR-020's project-wide convention, and `closes_on` is never rendered raw — `fmt_period()` only. **AC-219-05 A `CLOSED` cycle cannot be reopened.** It is a historical record that assessments, calibration outcomes and step changes point at; reopening silently changes what those records mean. A correction is a **named amendment on the cycle** with an actor and a reason, never a state reversal — the same ruling as an occupied job level's ordinal (ADR-017b). **AC-219-06** Every transition is audited with actor, from-state and to-state; `CYCLE_OPENED` and `CYCLE_CLOSED` are `retention_class='EMPLOYMENT'`. **AC-219-07** The cycle is gated `performance:w` for authoring and `performance:r` for reading; the feature code is registered in **all four places** (migration, `seed_rbac.sql`, `setup_db.py`, plus default `role_feature_access`) or it does not exist outside one machine. **AC-219-08** Deleting a cycle is impossible once it has left `DRAFT`; a `DRAFT` cycle may be discarded and says so. **AC-219-09 No pay period reference anywhere on the cycle** — D-009(1) makes it independent of the pay round, which keeps EP43's dependency one-directional and stops a rating ever reaching an amount by way of a shared key. **AC-219-10** Empty state: a company with no cycle sees an explanatory screen naming who can open one, never a blank table. | |
| | **📋 ACCEPTANCE CRITERIA — KAN-220, written 10 Aug 2026 (D-009).** **AC-220-01 Eligibility is a SNAPSHOT taken when the cycle opens, not a live query.** This is the load-bearing decision in the story. A live query means somebody who joins mid-cycle silently appears in a manager's list, somebody who leaves silently vanishes, and the completion meter moves for reasons nobody did — which is indistinguishable from a bug. The snapshot is re-evaluated only by an **explicit, audited action** that reports what changed. **AC-220-02** Every ACTIVE employee of the company is evaluated, and each exclusion carries a **named reason** — `NEW_JOINER`, `LEAVER`, `EXCLUDED_EMPLOYMENT_TYPE`, `NO_MANAGER`, `HR_EXCLUDED`. A silent exclusion is the defect this story exists to prevent. **AC-220-03** A **joiner** after the window opens is included if they joined at least *N* days before it closes (company-configurable, default 90); otherwise excluded as `NEW_JOINER`, listed, and eligible next year. **AC-220-04** A **leaver** whose last day falls before the window closes is excluded as `LEAVER`, **and anything already written about them is retained** — an assessment is a record of a conversation that happened, not a work item to be cleaned up. **AC-220-05** A **part-cycle participant** is included and flagged `PARTIAL`, so a manager knows they are assessing a shorter period; the flag is shown at the point of assessment, never only in a report. **AC-220-06 An employee with no solid-line manager is `NO_MANAGER`**, and this is surfaced as a **blocker to opening the cycle**, not a footnote: there is nobody to assess them, so including them silently guarantees an incomplete round. Same family as KAN-201's no-dispositioner warning. It names the people and offers the org chart. **AC-220-07** `employment_type` exclusions are **company-configurable** with `CONTRACTOR` excluded by default; the default is shown *as* a default. **AC-220-08 Probation is NOT a cycle type (D-009(2)).** A probationer is in the annual cycle like everybody else. There is no probation entity, no probation cycle and no probation screen, and *"this step change happened at a probation review"* stays a free choice of `review_context` on EP42's KAN-192. **AC-220-09** HR may include or exclude an individual with a **mandatory reason**, audited, and the override is visible on the participant list — an unexplained exclusion from a review round is exactly what a works council or a tribunal asks about. **AC-220-10** Coverage is reported as a **named figure with its denominator** (D7) — *"142 of 147 participating; 5 excluded"* — with the excluded **listed by name and reason**, never a bare percentage. **AC-220-11** Row-scoped reads: a manager sees their own reports' eligibility, HR sees the company's, and nobody sees another tenant's. **AC-220-12** Nothing here derives from pay, and nothing here carries a rating — the participant list is who is in the round, not a judgement about them. | |
| | **✅ KAN-219 + KAN-220 DONE (10 Aug 2026) — EP44 P0 builds.** The two load-bearing decisions are enforced where they cannot be bypassed. **At most ONE round that is not CLOSED**, via a partial unique index rather than service code — two would make *"which round am I in?"* ambiguous and every downstream reader would pick one arbitrarily; the service also refuses it with a sentence naming the blocking round instead of a constraint violation. **Forward only, and a CLOSED round is never reopened** — assessments, calibration outcomes and step changes point at it, so reopening silently changes what those records mean; the screen offers only the *next* stage and the close confirmation states the irreversibility rather than asking "are you sure?". **Eligibility is a SNAPSHOT, not a live query** — live participation means a mid-round joiner silently appears in a manager's list, a leaver silently vanishes, and the completion meter moves for reasons nobody did, which is indistinguishable from a bug; re-evaluation is an explicit action that **reports what changed**, and **an HR override survives it untouched**. **Every exclusion carries a named reason**, closed in the database, with an exclusion-without-a-reason made impossible by a CHECK. **`NO_MANAGER` BLOCKS the round from opening** and names the people — verified against real data by closing one reporting line: *"1 person has no line manager, so nobody can assess them: Ingrid Mäkinen."* **Two entry conditions** both exist to stop a round that cannot finish: nobody unassessable before `OPEN`, and a **self-assessment deadline** before `IN_REVIEW` (OQ-10 — without it a silent employee deadlocks their own review *and* the round). **The eligibility policy lives on the ROUND, not a settings table**, so a 2027 round's exclusions stay explicable in 2029 — a settings table answers "what is the policy now?" when the question is "what was it then?". **No `period_type` column** (annual, D-009(1)): a configurable field with one legal value is a lie about what the product supports. **No pay reference anywhere**, tested against both the migration and the service's SQL, keeping the compensation dependency one-directional. Verified in a browser: *"45 of 46 people would be in this round, 1 on a shorter period. Nothing is frozen until you open it."* **4,947 pytest · browser 110/110 · vacation 39/39**, plus a CI-style rebuild. ⚠ **One permission question left OPEN on purpose:** a manager writing an assessment must not need the admin grant — the CFL-42-35 shape again — raised against KAN-224/225 rather than settled by inventing a sixth feature code early. | |
...
| | **📋 WHAT D-009 SETTLES ACROSS P1–P2, written 10 Aug 2026.** Recorded once here rather than repeated in ten stories, and recorded at all because each of these was an **assumption engineering would otherwise have made silently**. **(a) Annual cadence → KAN-221/222.** A goal belongs to exactly one cycle and therefore to one year; there is no rollover and **no automatic carry-forward** — carrying an unmet goal into next year is a decision somebody makes, and a system that did it automatically would be asserting the goal is still agreed. Re-authoring next year's goal from last year's text is a **copy with an explicit action**, and the copy cites what it came from. KAN-222's mid-term check-in is a *moment inside* the annual cycle, not a second cycle. **(b) Annual cadence → KAN-226/227.** The rating scale and the review form are **versioned per cycle** and, because a cycle is a year, a version is in force for a year: *"a rating from 2027 must still be readable in 2029 against the scale that produced it"* becomes a concrete requirement — every stored rating carries its scale version id, and the scale is **immutable once a cycle using it has left `DRAFT`**. Editing a live scale silently re-labels ratings already given. **(c) Probation is a label → KAN-225.** A manager's assessment has **no probation variant**, no shorter form and no probation outcome field. If a report happens to be on probation the assessment is the ordinary one; the probation conversation, if the company has one, lives outside this product. **(d) Visibility → KAN-224/225/228, and this is the sharpest consequence.** The order is **fixed and enforced, not merely encouraged**: the employee's self-assessment must be submitted before the manager's assessment is visible to the employee, and the manager's assessment becomes visible to the employee **on submission** (D-009(3)). Three things follow. **(i)** The manager's assessment needs a **deliberate submit step** distinct from saving a draft — a manager must be able to write, leave, and reconsider without the employee reading it mid-thought, so *save* and *submit* are different verbs with different buttons. **(ii)** Once submitted it is **visible and no longer freely editable**: a correction is an audited amendment with a reason, because silently rewriting an assessment the employee has already read is worse than being wrong in public. **(iii)** KAN-228's button still reads **`Confirm we discussed this`** and never *"Accept"* — the employee seeing the assessment does not mean they agree with it, and a label that implies agreement is a false claim (standing rule 6). **(e) Not in scope for any of these, because A6 settled it:** nothing in P1–P2 may compute an amount, carry a currency, or map a rating to money. The review justifies the **step**, the step determines base pay. `TestNoAssessmentColumnsOnTheLadder` guards the reverse direction already; the equivalent guard here is that **no EP44 table may hold a pay column**. | |
